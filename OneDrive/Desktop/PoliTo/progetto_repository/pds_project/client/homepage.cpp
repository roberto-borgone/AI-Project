    ...
    : QWidget(parent), mw(mw)
{
    ...
    user->setText("<font color=\"black\">User: </font> <font color=\"green\">"+MainWindow::username+"</font>");
    ...
    createButton->setFixedSize(findButton->size());
    deleteFile->setFixedSize(findButton->size());

    URI = new QLineEdit(this);
    URI->setMaxLength(32);
    URI->setPlaceholderText(tr("Insert here the URI"));
    fileComboBox->setFixedHeight(fileComboBox->height());
    fileComboBox->lineEdit()->setPlaceholderText("Insert here the name of the file");
    ...
    /*
    mainLayout3->addWidget(filesFoundLabel, 5, 1);
    mainLayout3->addWidget(createButton,5, 2, Qt::AlignRight);
    mainLayout3->addWidget(deleteFile, 5, 3);
    mainLayout3->addWidget(findButton, 5, 4);
    */
    ...
    //DIALOG DI FIANCO A NEW FILE
    /*
    mainLayout->addWidget(createButton,5, 3, Qt::AlignRight);
    mainLayout->addWidget(findButton, 5, 4);
    mainLayout->addWidget(filesFoundLabel, 5, 1);
    */
    ...
    //mainLayout2->addWidget(spazio,5,0);
...
void Window::openFile(QString filename, QString owner, int siteid)
{
    ...
    textedit->addUserLabel(textedit->getSiteID(),MainWindow::username);
    textedit->show();
    //textedit->load(filename);
    qDebug()<<textedit->getSiteID();
...
void Window::openFileOfItem(int row, int /* column */)
{
    ...
    //textedit->setWindowTitle(fileNameOfItem(item));
    //textedit ->show();
    //textedit->load(fileNameOfItem(item));
    //textedit->setSiteID(1);
    //textedit->setOwner(fileNameOfItem(filesTable->item(row,2)));
    //textedit->addUserLabel(textedit->getSiteID(),MainWindow::username);
    //siteID
    //qDebug()<<textedit->getSiteID();
}
