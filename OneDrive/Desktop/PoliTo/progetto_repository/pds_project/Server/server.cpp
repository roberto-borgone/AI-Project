    ...
    QObject(parent)
{
    ...
    this->_siteID = 0;
    //this->path = "C:/Users/gianl/Desktop/pds_project_merge_24_04/Server/FileSystem/";
    //this->path = "C:/Users/Riccardo/Documents/pds_project/Server/FileSystem/";
    this->path = "C:/Users/rebec/OneDrive/Desktop/PoliTo/progetto_working_area/pds_project/Server/FileSystem/";


    //ThreadPool tp = ThreadPool();
    //nella newConnection:
    //tp.queueWork(connection, request);
    //mettere tp.queueWork(connection, request);
    //mettere la readyRead in processRequest e mettere come pubbliche find, edit, createNewFile, sendUri, share
    //altrimenti non posso chiamarle da processRequest in quanto private
    //oppure ThreadPool eredita da Server, si mette readyRead, find, edit, createNewFile, sendUri, share
    //come protected e si chiama la readyRead da processRequest in threadPool


    //QThreadPool
    //QThreadPool *pool = new QThreadPool(this);
    //setMaxThreadCount(10);
    //nella newConnection metto:
    //MyRunnable *task = new MyRunnable();
    //task->setAutoDelete(true);
    //connect(socket, SIGNAL(readyRead(),this,SLOT(pool->start(task)); //non funziona start non Ã¨ una slot
    //senza connect => pool->start(task)
    //il metodo run della classe MyRunnable contiene la readyRead
    ...
    if(!server->listen(QHostAddress::LocalHost, 9950))
    {
    ...
    if(socket.contains(client)){
       ...
       // socket->waitForReadyRead();
        ...
        if((proto.size()==8) && QString(proto).toUtf8().compare("+LOG    ")==0){
              ...
              query1.bindValue(0, username);
              query1.bindValue(1,QString(QCryptographicHash::hash((psw.toUtf8()),QCryptographicHash::Md5).toHex()));
              if(query1.exec() == false) {
                  array.clear();
                  array.insert(0,"-ERR    ;");
                  client->write(array);
           ...
           if (query.first() || query.value(nameCol1).toString()!=""){
               ...
               //! [4]
                   ...
                   // files.sort();
                    ...
                    client->write(array+";"); //di solito sono 2 byte perchÃƒÂ¨ il numero entra in 1 byte + 1 byte per \0
                    qDebug() <<"Dim: "<<array;
                    array.clear();
...
void Server::edit()
{
   ...
   if(oldPassword=="error" && newPassword=="error" && confirmPassword=="error")
       ...
       query1.bindValue(0, username);
       query1.bindValue(1, QString(QCryptographicHash::hash((oldPassword.toUtf8()),QCryptographicHash::Md5).toHex()));
           ...
           if (!query1.next()) {
               ...
               if(re.exactMatch(newPassword) == false){
                   ...
                   if(newPassword.compare(confirmPassword) != 0){
                       ...
                       query2.bindValue(0, QString(QCryptographicHash::hash((newPassword.toUtf8()),QCryptographicHash::Md5).toHex()));
...
void Server::insert()
{
    ...
    data.insert(0,"LI      ;"+stringSize+";,"); //messaggio da inviare
    ...
    QString op = "LI";
    QString username = data2[1];
    QString title = data2[2];
    int id = data2[3].toInt();
    int counter_message = data2[4].toInt();
    QString charValue = data2[5];
    QStringList posList = data2[6].split("|");
    std::vector<int> pos_message;
    for(auto i = 0; i<posList.size(); i++){
        pos_message.push_back(posList[i].toInt());
    }
    int x = data2[9].toInt();
    int y = data2[10].toInt();
    /*
    qDebug()<<op;
    qDebug()<<username;
    qDebug()<<ownerTitle;
    ...
    if(op == "LI") {
        ...
        if(size==1){ //se ho un solo simbolo
            ...
            if(localMap.find(key)->at(size-1).getPos()[0]<symbol.getPos()[0]){
                ...
                localMap.find(key)->push_back(symbol); //se il primo digit del mio simbolo ÃƒÂ¨ maggiore del primo dell'ultimo simbolo inserito(che ÃƒÂ¨ il maggiore) aggiungo in coda
            }
            ...
            /*
                ...
                else localMap.find(key)->push_back(symbol);
            }
      }
       ...
       */
       ...
       int nChar =1;
       if(multi==true) nChar = numChar.toInt();
       qDebug()<<"numero di caratteri"<<nChar<<"flag "<<multi;
       multi=false;

       QString messageLI = "LI;,"+QString::number(nChar)+";,"+QString::number(index)+";,"+symbol.toStr()+";,"+QString::number(x)+";,"+QString::number(y)+";,";
       ...
       QString msg = "IS      ;"+messageLI;//supportSize+messageLI;
       ...
       /*std::vector<QString> clientList  = clientMap.at(key); //
           for(size_t i=0; i<clientList.size(); i++){
               if(clientList.at(i) != username){
                   if(socketMap[clientList.at(i)]->write(msg.toUtf8()) != msg.size()){
       ...
       for(size_t i=0; i<clientList.size(); i++){
           if(clientList.at(i) != username){
               if(socketMap[clientList.at(i)]->write(msg.toUtf8()) != msg.size()){ //ci vuole il for perchè devo mandarlo a tutti
                   socketMap[clientList.at(i)]->write("Error sending symbols");
        ...
        */
        ...
        qDebug()<<"numero di caratteri"<<nChar<<"flag "<<multi;
        multi=false;
        QString messageLE = "LE;,"+QString::number(nChar)+";,"+QString::number(index)+";,"+symbol.toStr()+";,"+QString::number(x)+";,"+QString::number(y)+";,";
        /*auto size = messageLE.toUtf8().size();
        ...
        QString msg = "IS      ;"+messageLE;//supportSize+messageLE;
...
void Server::changePosition()
{
    QString stringSize = client->read(3); //leggo le prime 3 cifre che contengono l'informazione sulla dimensione della stringa da leggere
    qDebug()<<"stringSize"<<stringSize;
    QString data = client->read(stringSize.toInt());

    data.insert(0,"CHANGE  ;"); //messaggio da inviare
    messageQueue.push(data);
    //dispatchMessage();
    qDebug()<<data;
...
void Server::changeExistingFormat()
{
    ...
    /*
    auto it = std::find(localMap.toStdMap().at(conc).begin(),localMap.toStdMap().at(conc).end(), symbol);
    qDebug()<<"it"<<it->toStr();
    it->setFont(font);
    */
    /*
    for(auto i = localMap.find(conc)->begin(); i!= localMap.find(conc)->end(); i++){
    qDebug()<<i->toStr();
    }
    */
    int i = 0;
    ...
    /*
    for(auto i = 0; i<localMap.toStdMap().at(conc).size(); i++){
        if(i == index){

            auto app = localMap.toStdMap().at(conc).at(index.toInt());
        }
    }
    */
    //localMap.toStdMap().at(conc).at(index.toInt()).setFont(font);
    qDebug()<<localMap.toStdMap().at(conc).at(index.toInt()).toStr();
    //dispatchMessage();
    QString fontInsert = "TCFM    ;"+owner+";,"+title+";,"+index+";,"+font.toString();
    qDebug()<<"front "<<fontInsert;
    std::vector<QString> clientList  = clientMap.at(owner+title); //
    for(size_t i=0; i<clientList.size(); i++){
        if(clientList.at(i) != username){
...
{
    numChar = client->read(3);   //leggo le prime 3 cifre che contengono l'informazione sul numero di caratteri da leggere
    qDebug()<<numChar;
    multi =true;
...
void Server::dispatchMessage()
{
    while(!messageQueue.empty()){
        ...
        QString username = message.split(";,")[1];
        QString filename = message.split(";,")[2];
        QString owner = message.split(";,")[8]; //supponendo che l'uri sia in seconda posizione xd
        std::vector<QString> clientList  = clientMap.at(owner+filename);
        for(size_t i=0; i<clientList.size(); i++){
            if(clientList.at(i) != username){
...
void Server::fileOpen(QString filename, QString owner, QString username)
{
    ...
    if(QFile(path+"/Documenti/"+owner+"/"+filename+".txt").exists() == true){
        ...
        //array.clear();
        int siteID;
        if(localMap.contains(owner+filename)){
...
void Server::discoNoSlot()
{
    ...
    if(localMap.find(keyOwnerTitle).value().empty()){
        ...
        if(clientMap.at(keyOwnerTitle).empty()){
            ...
            }while(localMap.find(keyOwnerTitle)->size()!=file.size() && counter!=fileFont.size());
...
//inizializzare tutti i puntatori a nullptr nei costruttori oppure usare smart pointer
//copia-incolla e undo-redo non funzionano con grande numero di caratteri (sopra 15)
//aggiustare il tempo delle sleep server e client (local insert e local erase)
//salvataggio del file con più client che è andato in loop e ha scritoo 800000 volte ciaomamm -> aggiungere var per evitare loop infinito nel do while
